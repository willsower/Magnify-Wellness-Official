/*
Populates app.js page

  Current Structure
      reviews = [
        {
          name: "",
          reviewName: "",
          description: "",
          stars: #,
        },
      ]
      
  - name refers to the name of the person leaving the review
  - reviewName is the title the user added
  - description is the description
  - stars is how many stars the user had (example 4 if 4/5 or 5 if 5/5) this is int

  TO ADD ANOTHER ITEM, COPY THE FOLLOWING
      {
          name: "",
          reviewName: "",
          description: "",
          stars: #,
      }
    ADD THE ABOVE JSON ITEM WITHIN THE CORRESPONDING SECTION
*/

export const reviews = [
  {
    name: "Sparkly Cupcakes",
    reviewName: "Great mental health resource!",
    description:
      "Maggie is the cutest chatbots. The rest of the app, from the journal, to the small games, to the quotes, is brilliant! Thank you!",
    stars: 5,
  },
  {
    name: "Jessicaa Z",
    reviewName: "It's really helpful!",
    description:
      "LOVE the journal feature, and I can tell so much effort was put into this app!! Really impressed by all the features! :)",
    stars: 5,
  },
  {
    name: "Ashlaurenryan",
    reviewName: "Amazing App",
    description:
      "This app is so important for anybody to use it allows you to think and reflect about your own thoughts and how to tackle any problems.",
    stars: 5,
  },
  {
    name: "Charliechaplan14",
    reviewName: "I definitely recommend it.",
    description:
      "| downloaded this app after hearing a presentation from the creator and I love it! It is so well designed and really helpful.",
    stars: 5,
  },
  {
    name: "mimijknychk88",
    reviewName: "I love this app",
    description: "I've been using the app for a while and i love its features!",
    stars: 4,
  },
  {
    name: "lindt lu",
    reviewName: "Good for Stress Relief",
    description:
      "i have been stressed due to work and personal obligations, and i was recccomended this app by a friend.I would recommend it to others.",
    stars: 5,
  },
];
